    ...
    override fun resetValidators(context: Context) {
        ...
        when (testType) {
            ...
            EditTextValidator.TEST_BG_RANGE            -> BgRangeValidator(if (TextUtils.isEmpty(testErrorString)) context.getString(R.string.error_only_numeric_digits_range_allowed,
                profileUtil.fromMgdlToUnits(minMgdl.toDouble()).toString(), profileUtil.fromMgdlToUnits(maxMgdl.toDouble()).toString()) else testErrorString, minMgdl, maxMgdl, profileUtil)
