...
class Widget : AppWidgetProvider() {
    ...
    private fun updateBg(views: RemoteViews) {
        views.setTextViewText(R.id.bg, overviewData.lastBg(iobCobCalculator.ads)?.let { profileUtil.fromMgdlToStringInUnits(it.value) } ?: rh.gs(info.nightscout.core.ui.R.string.value_unavailable_short))
        views.setTextColor(
            R.id.bg, when {
    ...
    private fun updateTemporaryTarget(views: RemoteViews) {
        ...
        if (tempTarget != null) {
            ...
            views.setTextViewText(R.id.temp_target, profileUtil.toTargetRangeString(tempTarget.lowTarget, tempTarget.highTarget, GlucoseUnit.MGDL, units) + " " + dateUtil.untilString(tempTarget.end, rh))
        } else {
